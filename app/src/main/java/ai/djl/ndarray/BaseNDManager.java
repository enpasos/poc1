...
public abstract class BaseNDManager implements NDManager {
    ...
    public void debugDump2(int level) {
        StringBuilder sb = new StringBuilder(100);
        for (int i = 0; i < level; ++i) {
            sb.append("    ");
        }
        ...
        for (AutoCloseable c : resources.values()) {
            if (c instanceof NDManager) {
                ((BaseNDManager) c).debugDump2(level + 1);
            } else if (c instanceof PtNDArray) {
                StringBuilder sb2 = new StringBuilder(100);
                for (int i = 0; i < level+1; ++i) {
                    sb2.append("    ");
                }
                sb2.append("\\--- NDArray(" + ((PtNDArray)c).getUid() + ", Shape"+ ((PtNDArray) c).getShape() + ")");
